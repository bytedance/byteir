...
template <typename Engine0, typename Layout0, typename Engine1,
          typename Layout1, typename TiledCopy>
inline __device__ void
write_softmax_to_gmem(Tensor<Engine0, Layout0> const &tOrP,
                      Tensor<Engine1, Layout1> &tPgP,
                      TiledCopy gmem_tiled_copy_P) {
  // Reshape tOrP from (8, MMA_M, MMA_N) to (8, MMA_M * MMA_N)
  Layout l = tOrP.layout();
  Tensor tPrP = make_tensor(
      tOrP.data(), make_layout(get<0>(l), make_layout(get<1>(l), get<2>(l))));
...
inline __device__ void compute_attn_1rowblock(const Params &params,
  ...
  constexpr int MMA_M =
      kBlockM / decltype(size<0>(
                    typename Kernel_traits::TiledMma::TiledShape_MNK{}))::value;
  ...
  if ((Is_causal || Is_local || !Is_even_MN) && n_block_max <= n_block_min) {
    // Save seed and offset for backward. If we don't have this here, the 0-th
    // thread block might exit early and no one saves the rng state.
    ...
    Tensor cO = make_identity_tensor(
        make_shape(size<0>(gO), size<1>(gO))); // (BLK_M,BLK_K) -> (blk_m,blk_k)
...
#pragma unroll
  ...
  Tensor scores_max =
      make_tensor<ElementAccum>(Shape<Int<2 * size<1>(acc_o)>>{});
...
#pragma unroll
  ...
  // anyway.
  flash::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_QKV, tKgK, tKsK, tKVcKV,
                                     tKVpKV,
  ...
  //       std::get<1>(seeds) + (bidb * params.h + bidh) * 32 + tidx % 32;
...
#pragma unroll
  for (int masking_step = 0; masking_step < n_masking_steps;
    ...
    // We don't put the masking before the matmul S = Q K^T because we don't
    // clear sK for rows outside actual_seqlen_k. So those rows could have Inf /
    // NaN, and the matmul can produce Inf / NaN.

    if (Has_alibi) {
      flash::apply_alibi<Is_causal>(
          scores, n_block * kBlockN, binfo.actual_seqlen_k,
          m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4,
          binfo.actual_seqlen_q, kNWarps * 16, alibi_slope);
    }
    ...
    if (!Is_causal && !Is_local) {
      ...
      // Tensor caccS = make_identity_tensor(Shape<Int<kBlockM>,
      // Int<kBlockN>>{});    // (BLK_M,BLK_N) -> (blk_m,blk_n) Tensor taccScS =
      // thr_mma.partition_C(caccS);                           //
      // (MMA,MMA_M,MMA_N) static_assert(decltype(size<0>(taccScS))::value ==
      // 4);
      // // Convert to ((2, 2), MMA_M, MMA_N) then take only the row indices.
      // Tensor idx_row = logical_divide(taccScS, Shape<_2>{})(make_coord(0, _),
      // _, 0); Tensor idx_rowcol = make_tensor(taccScS.data(),
      // flash::convert_layout_acc_rowcol(taccScS.layout()));
      // flash::apply_mask_causal_w_idx(scores, idx_rowcol, n_block * kBlockN,
      // binfo.actual_seqlen_k,
      //                               m_block * kBlockM);
      // Idk why it's get<1> and not get<0> of the stride.
      // if (cute::thread0()) { print(idx_row.layout());
      // print(stride<1>(idx_row)); printf("stride = %d \n",
      // get<1>(stride<1>(idx_row))); } I can't get the stride from idx_row
      flash::apply_mask_local</*HasWSLeft=*/Is_local>(
          scores, n_block * kBlockN, binfo.actual_seqlen_k,
          // m_block * kBlockM + get<0>(idx_row(0)),
          m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4,
          binfo.actual_seqlen_q, kNWarps * 16, params.window_size_left,
    ...
    // Reshape rP from (nrow=(2, MMA_M), ncol=(2, MMA_N)) to ((2, 2, 2), MMA_M,
    // MMA_N / 2) if using m16n8k16 or ((2, 2, 1), MMA_M, MMA_N) if using
    // m16n8k8.
    Tensor tOrP = make_tensor(
        rP.data(), flash::convert_layout_rowcol_Aregs<Kernel_traits::TiledMma>(
                       rP.layout()));
    ...
    if (Return_softmax) {
      ...
      flash::apply_dropout</*encode_dropout_in_sign_bit=*/true>(
          tOrP_copy, params.p_dropout_in_uint8_t, seed, offset, block_row_idx,
          block_col_idx, kNWarps);
  ...
  for (; n_block >= n_block_min; --n_block) {
    ...
    // Advance gV
    tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
    flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tVgV, tVsV,
                                                tKVcKV, tKVpKV);
    cute::cp_async_fence();
    ...
    if (Is_local && n_block * kBlockN <
                        (m_block + 1) * kBlockM + binfo.actual_seqlen_k -
                            binfo.actual_seqlen_q + params.window_size_right) {
    ...
    softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_local>(
        scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2);
...
#pragma unroll
  for (int mi = 0; mi < size<0>(acc_o_rowcol); ++mi) {
    ...
    lse(mi) = (sum == 0.f || sum != sum)
                  ? INFINITY
                  : scores_max(mi) * params.scale_softmax + __logf(sum);
...
template <typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi,
          bool Is_even_MN, bool Is_even_K, bool Split, bool Append_KV,
          typename Params>
inline __device__ void
compute_attn_1rowblock_splitkv(const Params &params, const int bidb,
  ...
  if (n_block_min >=
    ...
    Tensor gLSEaccum =
        make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(
...
#pragma unroll
  ...
  if constexpr (Append_KV) {
    ...
    Tensor tKgKnew =
        gmem_thr_copy_QKV.partition_S(gKnew); // (KCPY, KCPY_N, KCPY_K)
    Tensor tVgVnew =
        gmem_thr_copy_QKV.partition_S(gVnew); // (VCPY, VCPY_N, VCPY_K)
    ...
    if (n_block_max > n_block_copy_min) {
      tKgK.data() = tKgK.data() + (n_block_max - n_block_copy_min) * kBlockN *
                                      params.k_row_stride;
      tVgV.data() = tVgV.data() + (n_block_max - n_block_copy_min) * kBlockN *
                                      params.v_row_stride;
    }
  }
...
#pragma unroll
  for (int masking_step = 0; masking_step < n_masking_steps;
    ...
    masking_step == 0
        ? softmax_rescale_o</*Is_first=*/true,
                            /*Check_inf=*/Is_causal || Is_local || !Is_even_MN>(
              scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2)
        : softmax_rescale_o</*Is_first=*/false,
                            /*Check_inf=*/Is_causal || Is_local || !Is_even_MN>(
              scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2);
...
inline __device__ void compute_attn_splitkv(const Params &params) {
  ...
  flash::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local,
                                        Has_alibi, Is_even_MN, Is_even_K, Split,
                                        Append_KV>(params, bidb, bidh, m_block,
...
#pragma unroll
  for (int l = 0; l < kNLsePerThread; ++l) {
    ...
    ElementAccum lse =
        (row < params.num_splits &&
         col < params.b * params.h * params.seqlen_q - bidx * kBlockM)
            ? gLSEaccum(row, col)
            : -INFINITY;
    if (row < kMaxSplits) {
      sLSE[row][col] = lse;
