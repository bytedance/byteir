...
apply_mask_local(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
...
template <typename Engine, typename Layout>
inline __device__ void
apply_mask_causal(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
                  const int max_seqlen_k, const int row_idx_offset,
                  const int max_seqlen_q, const int warp_row_stride) {
...
template <typename Engine0, typename Layout0, typename Engine1,
          typename Layout1>
inline __device__ void
apply_mask_causal_w_idx(Tensor<Engine0, Layout0> &tensor,
...
template <bool encode_dropout_in_sign_bit = false, typename Engine,
          typename Layout>
inline __device__ void
apply_dropout(Tensor<Engine, Layout> &tensor, uint8_t p_dropout_in_uint8_t,
...
#pragma unroll
    for (int n = 0; n < size<2>(tensor) / 2; ++n, ++rowcol.y) {
      ...
      // Special implementation for 16-bit types: we duplicate the threshold to
      // the low and high 16 bits of a 32-bit value, then use the f16x2
      // comparison instruction to get a mask. The low 16 bits of the mask will
      // be either 0xffff or 0x0000, and the high 16 bits will be either 0xffff
      // or 0x0000, depending on whether the random value is less than the
      // threshold. We then do a bit-wise AND between the mask and the original
      // value (in 32-bit). We're exploiting the fact that floating point
      // comparison is equivalent to integer comparison, since we're comparing
      // unsigned integers whose top 8-bits are zero.
