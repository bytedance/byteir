...
struct Flash_fwd_kernel_traits : public Base {
  ...
  using TiledMma =
      TiledMMA<typename Base::MMA_Atom_Arch,
               Layout<Shape<Int<kNWarps>, _1, _1>>, // 4x1x1 or 8x1x1 thread
                                                    // group
               typename Base::ValLayoutMNK>; // 1x2x1 or 1x2x2 value group for
  ...
  using GmemTiledCopyQKV = decltype(
      make_tiled_copy(Copy_Atom<Gmem_copy_struct, Element>{}, GmemLayoutAtom{},
                      Layout<Shape<_1, _8>>{})); // Val layout, 8 vals per read
  using GmemTiledCopyO = decltype(
      make_tiled_copy(Copy_Atom<DefaultCopy, Element>{}, GmemLayoutAtom{},
  ...
  using GmemTiledCopyP = decltype(
      make_tiled_copy(Copy_Atom<DefaultCopy, Element>{}, GmemLayoutAtomP{},
                      Layout<Shape<_1, _8>>{})); // Val layout, 8 vals per store
...
template <int kHeadDim_, int kBlockM_, int kBlockN_, int kNWarps_,
          ...
          int AtomLayoutMdQ = 2, bool Is_V_in_regs_ = false,
          bool No_double_buffer_ = false, typename elem_type = cutlass::half_t,
          typename Base = Flash_kernel_traits<kHeadDim_, kBlockM_, kBlockN_,
                                              kNWarps_, elem_type>>
struct Flash_bwd_kernel_traits : public Base {
  ...
  using TiledMmaSdP = TiledMMA<
      ...
      typename Base::ValLayoutMNK>; // 1x2x1 or 1x2x2 value group for 16x16x16
                                    // MMA and LDSM
  ...
  using TiledMmadQ =
      TiledMMA<typename Base::MMA_Atom_Arch,
               Layout<Shape<Int<AtomLayoutMdQ>, Int<kNWarps / AtomLayoutMdQ>,
  ...
  using SmemLayoutQdO = decltype(tile_to_shape(
      SmemLayoutAtomQdO{}, make_shape(Int<kBlockM>{}, Int<kHeadDim>{})));
  ...
  // than 2 static constexpr int kPBlockN = kBlockN;
  ...
  static constexpr int kSmemQdOCount =
      size(SmemLayoutQdO{}) *
      (No_double_buffer ? 2 : 3); // Double buffer for sQ
  static constexpr int kSmemKVCount = size(SmemLayoutKV{}) * 2;
  ...
  static constexpr int kSmemSize =
      ...
      (!Is_V_in_regs
           ...
           : std::max(kSmemKVSize, kSmemKVSize / 2 + kSmemdSSize +
  ...
  static constexpr int kSmemSize1rowblock =
      kSmemQdOSize / 3 * 2 + kSmemKVSize / 2 * 3 + kSmemdSSize + kSmemPSize;
