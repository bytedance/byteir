...
make_tiled_copy_B_warpcontiguousN(Copy_Atom<Args...> const &copy_atom,
  ...
  constexpr int kNWarpsN =
      decltype(size<1>(TileShape_MNK{}))::value / AtomShape_N / 2;
  ...
  // if (cute::thread0()) {printf("make_tiled_copy_B_warpcontiguousN ");
  // print(t); printf("\n");  }
...
make_tiled_copy_C_warpcontiguousN(Copy_Atom<Args...> const &copy_atom,
  ...
  constexpr int MMAStride_N = MMA_N * AtomShape_N * 2;
  auto t = make_tile(
      make_layout(size<0>(TileShape_MNK{})),
      Layout<Shape<Int<AtomShape_N>, Int<kNWarpsN>, _2>, // (8, 2, 2) or (8, 4,
                                                         // 2)
             Stride<_1, Int<MMAStride_N>, _8>>{}); // (1, 64, 8) or (1, 32, 8)
  // if (cute::thread0()) {printf("make_tiled_copy_C_warpcontiguousN ");
  // print(t); printf("\n");  }
...
inline __device__ void dot_do_o(Tensor<Engine0, Layout0> const &do_,
  ...
  // Reshape do_ and o from (8, kBlockM / 32, kHeadDim / 64) to (kBlockM / 32, 8
  // * kHeadDim / 64) The last coordinate is the "page".
  Tensor do_reshaped = make_tensor(
      do_.data(),
      make_layout(get<1>(do_.layout()),
...
#pragma unroll
    ...
    dP_sum_cur =
        flash::Allreduce<THREADS_PER_ROW>::run(dP_sum_cur, sum_op) * scale;
...
inline __device__ void compute_dot_do_o(const Params &params) {
  ...
  const index_t row_offset_dq_accum =
      binfo.q_offset(params.seqlen_q_rounded * params.h * params.d_rounded,
                     params.h * params.d_rounded, bidb) +
  ...
  Tensor dP_sum = make_tensor(
      make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.dsoftmax_sum) +
                    row_offset_dpsum),
...
#pragma unroll
  ...
  flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false, /*Clear_OOB_MN=*/true>(
      gmem_tiled_copy_dO, tdOgdO, tdOrdO, tdOcdO, tdOpdO,
      binfo.actual_seqlen_q - m_block * kBlockM);
  flash::copy</*Is_even_MN=*/false, /*Is_even_K=*/false, /*Clear_OOB_MN=*/true>(
      gmem_tiled_copy_dO, tdOgO, tdOrO, tdOcdO, tdOpdO,
      binfo.actual_seqlen_q - m_block * kBlockM);
  // By right we need to scale dP up by 1/p_dropout, but instead we don't and
  // only scale the final results (dQ and dK) by 1/p_dropout. So we need to keep
  // dP_sum scaled down by p_dropout here, so that (dP - dP_sum) is on the same
  // scale.
...
inline __device__ void clear_dKVaccum(const Params &params) {
  ...
  const index_t row_offset_dkv_accum =
      ((bidb * params.h_k + bidh) * params.seqlen_k_rounded +
       n_block * kBlockN) *
      params.d_rounded;

  Tensor gdKaccum = make_tensor(
      make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.dk_accum_ptr) +
                    row_offset_dkv_accum),
      Shape<Int<kBlockN>, Int<kHeadDim>>{}, Stride<Int<kHeadDim>, _1>{});
  Tensor gdVaccum = make_tensor(
      make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.dv_accum_ptr) +
                    row_offset_dkv_accum),
      Shape<Int<kBlockN>, Int<kHeadDim>>{}, Stride<Int<kHeadDim>, _1>{});
...
// This is used in the case where we want to parallelize the backward across
// seqlen_k.
template <typename Kernel_traits, typename Params>
...
inline __device__ void convert_dKV(const Params &params) {
  ...
  Tensor acc_dk = partition_fragment_C(
      tiled_mma_dkv, Shape<Int<kBlockN>, Int<kHeadDim>>{}); // MMA, MMA_N, MMA_K
...
#pragma unroll
  ...
  Tensor cdKV = make_identity_tensor(
      Shape<Int<kBlockN>, Int<kHeadDim>>{}); // (BLK_M,BLK_K) -> (blk_m,blk_k)
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          ...
          bool Is_first, bool Is_last, bool Seq_parallel = false,
          typename Params>
inline __device__ void
compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh,
                           const int n_block) {
  ...
  constexpr int MMA_N_SdP =
      kBlockN /
      decltype(size<1>(
          typename Kernel_traits::TiledMmaSdP::TiledShape_MNK{}))::value;
  ...
  if (Is_local) {
    m_block_max = std::min(
        m_block_max,
        cute::ceil_div((n_block + 1) * kBlockN + binfo.actual_seqlen_q -
  ...
  const index_t row_offset_q =
      ...
      (m_block_max - 1) * kBlockM * params.q_row_stride +
  ...
  const index_t row_offset_o =
      binfo.q_offset(params.o_batch_stride, params.o_row_stride, bidb) +
      (m_block_max - 1) * kBlockM * params.o_row_stride +
  ...
  const index_t row_offset_dq_accum =
      ...
      ((m_block_max - 1) * kBlockM +
       (params.cu_seqlens_q == nullptr ? 0 : 128 * bidb)) *
          params.h * params.d_rounded +
      bidh * params.d_rounded
  ...
  Tensor sdS = make_tensor(!Kernel_traits::Is_V_in_regs ? sV.data() + size(sV)
                                                        : sK.data() + size(sK),
  ...
  using GmemTiledCopydO =
      std::conditional_t<Is_first, typename Kernel_traits::GmemTiledCopydO,
  ...
  using GmemLayoutAtomdQaccum =
      std::conditional_t<!Seq_parallel,
  ...
  Tensor tdVrdO = thr_mma_dkv.partition_fragment_B(
      sdOtransposedNoSwizzle); // (MMA, MMA_K, MMA_N)
  ...
  // auto smem_thr_copy_KV = make_tiled_copy_B(typename
  // Kernel_traits::SmemCopyAtom{}, tiled_mma_sdp).get_thread_slice(tidx);
  ...
  // if (cute::thread(0, 0) && n_block == 0) { printf("sK layout: ");
  // print(sK.layout()); printf("\n"); } if (cute::thread(0, 0) && n_block == 0)
  ...
  // auto smem_thr_copy_PdS = make_tiled_copy_C(typename
  // Kernel_traits::SmemCopyAtomPdS{}, tiled_mma_sdp).get_thread_slice(tidx);
  ...
  // if (cute::thread(0, 0) && n_block == 0) { printf("sP layout: ");
  // print(sP.layout()); printf("\n"); } if (cute::thread(0, 0) && n_block == 0)
  // { print(tPsP.layout()); printf("\n"); } if (n_block == 0 && blockIdx.x == 0
  // && blockIdx.y == 0 && tidx < 64) {
...
#pragma unroll
  ...
  tdQgdQaccum.data() =
      tdQgdQaccum.data() + kBlockM * params.h * params.d_rounded;
  ...
  // causal case, n_block * kBlockN + binfo.actual_seqlen_q -
  // binfo.actual_seqlen_k < actual_seqlen_q. So m_block_min <= (actual_seqlen_q
  // - 1) / kBlockM. Recall that m_block_max =
  // cute::ceil_div(binfo.actual_seqlen_q, kBlockM) = (actual_seqlen_q + kBlockM
...
#pragma unroll
  ...
  if (!Is_first) {
    ...
    flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
        ...
        binfo.actual_seqlen_q - m_block * kBlockM);
    flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
        gmem_tiled_copy_dO, tdOgO, tdOrO, tQcQ, tQpQ,
        binfo.actual_seqlen_q - m_block * kBlockM);
  }
  flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
  ...
  Tensor caccS = make_identity_tensor(
      Shape<Int<kBlockM>, Int<kBlockN>>{}); // (BLK_M,BLK_N) -> (blk_m,blk_n)
  ...
  Tensor taccScS_row =
      logical_divide(taccScS, Shape<_2>{})(make_coord(0, _), _, 0);
  Tensor lse = make_tensor<ElementAccum>(
      Shape<Int<decltype(size(taccScS_row))::value>>{});
#pragma unroll
  ...
  // We want LSE = inf if the row is OOB. In that case Q would be zero, K would
  // be zero, and scores would be zero. With LSE = 0, probs will be all 1's, and
  // when we multiply with V (which would be zero), we're fine. However, with
  // ALiBi, we might modify these scores, and probs can become NaN. Instead if
  // we set LSE = inf for OOB rows, probs are always 0.
...
#pragma unroll
    ...
    //     cute::copy(smem_tiled_copy_KV, tSsK(_, _, k), tSrK_copy_view(_, _,
    //     k));
    // }
    ...
    // Reshape acc_s from (MMA=4, MMA_N, MMA_N) to (col=(2, MMA_N), row=(2,
    // MMA_N))
    ...
    if (Has_alibi) {
      flash::apply_alibi<Is_causal>(
          ...
          binfo.actual_seqlen_q, AtomLayoutMS * 16, alibi_slope);
    }

    // TD [2023-07-29]: I was thinking that we don't need to mask out the
    // elements beyond actual_seqlen_k, because acc_s would be some finite value
    // for those indices. In the end when we multiply with K to get dQ, the
    // corresponding values of K would be 0, so the result would still be
    // correct. However, it's possible that the values in acc_s are so large
    // that they overflow when we multiply with dP and convert to fp16,
    // resulting in Inf in dS and NaNs in dQ. So we need to mask out the
    // elements beyond actual_seqlen_k.
    if (!Is_causal && !Is_local) {
      ...
      // Putting this causal masking right after acc_s is *much* slower for some
      // reason. TD [2023-08-16]: We need the 2nd condition because if seqlen_q
      // is long and seqlen_k is short (e.g., 256 and 2), the 2nd block of
      // seqlen_q (from 128 to 255), we're not doing causal masking. But we
      // still want to mask out elements beyond actual_seqlen_k.
      if (m_block * kBlockM < (n_block + 1) * kBlockN + binfo.actual_seqlen_q -
        ...
        flash::apply_mask_causal(
            ...
            // binfo.actual_seqlen_k, m_block * kBlockM + (tidx / 32) %
            // AtomLayoutMS * 16 + (tidx % 32) / 4,
            AtomLayoutMS * 16);
      }
    } else if (Is_local) {
      if (m_block * kBlockM < (n_block + 1) * kBlockN + binfo.actual_seqlen_q -
          ...
          (m_block + 1) * kBlockM >= n_block * kBlockN + binfo.actual_seqlen_q -
                                         binfo.actual_seqlen_k +
    ...
    flash::scale_apply_exp2</*scale_max=*/false>(scores, lse,
                                                 params.scale_softmax_log2);
    if (Is_dropout) {
      int warp_id = tidx / 32;
      int block_row_idx = m_block * (kBlockM / 16) + warp_id % AtomLayoutMS;
      // Need col to be multiples of 32, since we're doing dropout with block of
      // 16 x 32
      static_assert(MMA_N_SdP % 2 == 0);
      int block_col_idx =
          n_block * (kBlockN / 32) + (warp_id / AtomLayoutMS) * (MMA_N_SdP / 2);
      Tensor scores_dropped = make_tensor(
          scores.data(),
          flash::convert_layout_rowcol_Aregs<Kernel_traits::TiledMmaSdP>(
              scores.layout()));
    ...
    Tensor rP = !Is_dropout ? flash::convert_type<Element>(scores)
                            : flash::convert_type_relu<Element>(scores);
    // Reshape rP from (nrow=(2, MMA_N), ncol=(2, MMA_N)) to ((2, 2, 2), MMA_N,
    // MMA_N / 2) if using m16n8k16 or ((2, 2, 1), MMA_N, MMA_N) if using
    // m16n8k8.
    Tensor tPrP = make_tensor(
        ...
        flash::convert_layout_rowcol_Aregs<Kernel_traits::TiledMmaSdP>(
            rP.layout()));
    Tensor tPaP =
        smem_thr_copy_PdS.retile_S(tPrP); // ((Atom,AtomNum), MMA_N, MMA_N)
    ...
    clear(acc_dp);
    // Tensor acc_dp_reshaped = make_tensor(acc_dp.data(),
    // flash::convert_layout_acc_rowcol(acc_dp.layout())); #pragma unroll for
    ...
    // Reshape acc_dp from (MMA=4, MMA_N, MMA_N) to (col=(2, MMA_N), row=(2,
    // MMA_N))
...
#pragma unroll
    ...
    tdQgdQaccum.data() =
        tdQgdQaccum.data() + (-int(kBlockM * params.h * params.d_rounded));
    if (Is_first || Seq_parallel) {
      clear(acc_dq);
    } else {
      // Reshape acc_dq from (4, 1, 2) to (4, 2, 1) to write to gdQaccum
      Tensor acc_dq_reshaped =
          make_tensor(acc_dq.data(), make_layout(get<0>(acc_dq.layout()),
                                                 get<2>(acc_dq.layout()),
                                                 get<1>(acc_dq.layout())));
    ...
    if (Double_buffer && m_block > m_block_min) {
      ...
      flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tQgQ,
                                                  tQsQ, tQcQ, tQpQ);
      flash::cp_async_fence();
    }
    ...
    // Tensor tdVrPt = make_tensor(tPrP.data(), make_layout(get<0>(p_l),
    // get<2>(p_l), get<1>(p_l))); flash::gemm_A_in_regs(acc_dv, tdVrPt, tdVrdO,
    // tdVsdOt, tiled_mma_dkv, smem_thr_copy_QdOt); Tensor tdKrdSt =
    // make_tensor(tdSrdS.data(), tdVrPt.layout());
    // flash::gemm_A_in_regs(acc_dk, tdKrdSt, tdKrQt, tdKsQt, tiled_mma_dkv,
    // smem_thr_copy_QdOt);
    ...
    if (m_block > m_block_min) {
      ...
      if (Is_first) {
        ...
        flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_dO, tdOgdO,
                                                    tdOrdO, tQcQ, tQpQ);
        flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_dO, tdOgO,
                                                    tdOrO, tQcQ, tQpQ);
      } else {
        flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_dO, tdOgdO,
                                                    tdOsdO, tQcQ, tQpQ);
...
#pragma unroll
    ...
    if (Is_last) {
      ...
      Tensor cdQ = make_identity_tensor(
          Shape<Int<kBlockM>,
                Int<kHeadDim>>{}); // (BLK_M,BLK_K) -> (blk_m,blk_k)
      Tensor tdQcdQ = gmem_thr_copy_dQ.partition_D(cdQ);
#pragma unroll
      for (int m = 0; m < size<1>(tdQgdQ); ++m) {
        if (Is_even_MN || get<0>(tdQcdQ(0, m, 0)) <
                              binfo.actual_seqlen_q - m_block * kBlockM) {
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          bool Has_alibi, bool Is_even_N, bool Is_even_K, typename Params>
inline __device__ void
compute_dq_dk_dv_1rowblock(const Params &params, const int bidb, const int bidh,
                           const int m_block) {
  ...
  if (Is_causal) {
    n_block_max =
        std::min(n_block_max, cute::ceil_div((m_block + 1) * kBlockM, kBlockN));
  }
  ...
  const index_t row_offset_dkv_accum =
      ((bidb * params.h_k + (bidh / params.h_h_k_ratio)) *
  ...
  Tensor sQtNoSwizzle = make_tensor(
      sQ.data(), typename Kernel_traits::SmemLayoutQdOtransposedNoSwizzle{});
  Tensor sdO = make_tensor(sQ.data() + size(sQ),
                           typename Kernel_traits::SmemLayoutQdO{});
  Tensor sdOt = make_tensor(sdO.data(),
                            typename Kernel_traits::SmemLayoutQdOtransposed{});
  Tensor sdOtransposedNoSwizzle = make_tensor(
  ...
  Tensor sV = make_tensor(sK.data() + 2 * size(sK),
                          typename Kernel_traits::SmemLayoutKV{});
  Tensor sKt =
      make_tensor(sK.data(), typename Kernel_traits::SmemLayoutKtransposed{});
  Tensor sKtNoSwizzle = make_tensor(
      sK.data(), typename Kernel_traits::SmemLayoutKtransposedNoSwizzle{});
  Tensor sdS = make_tensor(sV.data() + size(sV),
                           typename Kernel_traits::SmemLayoutPdS{});
  Tensor sdSt = make_tensor(sdS.data(),
                            typename Kernel_traits::SmemLayoutPdStransposed{});
  Tensor sdStNoSwizzle = make_tensor(
  ...
  Tensor sdPsum = make_tensor(
      make_smem_ptr(reinterpret_cast<ElementAccum *>(sdS.data().get())),
      Shape<Int<kBlockM>>{});
  ...
  Tensor acc_dq = partition_fragment_C(
      tiled_mma_dq,
      Shape<Int<kBlockM>, Int<kHeadDim>>{}); // MMA, MMA_M_SdP, MMA_K
...
#pragma unroll
  for (int mi = 0; mi < size(dP_sum); ++mi) {
    dP_sum(mi) = sdPsum(get<0>(taccScS_row(mi)));
  }
  ...
  for (; n_block >= 0; --n_block) {
    Tensor acc_s = partition_fragment_C(
        tiled_mma_sdp,
        Shape<Int<kBlockM>, Int<kBlockN>>{}); // (MMA=4, MMA_M_SdP, MMA_N)
    clear(acc_s);
    ...
    // We don't need to mask out the elements beyond actual_seqlen_k, because
    // acc_s would be some finite value for those indices. In the end when we
    // multiply with K to get dQ, the corresponding values of K would be 0, so
    // the result would still be correct.
...
#pragma unroll
    ...
    if (n_block > 0) {
      ...
      flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tKgK,
                                                  tKsK, tKVcKV, tKVpKV);
      flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tVgV,
                                                  tVsV, tKVcKV, tKVpKV);
    ...
    // print(acc_dv); }
    tdVgdVaccum.data() =
        tdVgdVaccum.data() + (-int(kBlockN * params.d_rounded));
#pragma unroll
    for (int i = 0; i < size(acc_dv); ++i) {
      atomicAdd(&tdVgdVaccum(i), acc_dv(i));
    }
    ...
    flash::gemm(acc_dk, tdKrdSt, tdKrQt, tdKsdSt, tdKsQt, tiled_mma_dkv,
                ...
                smem_thr_copy_QdOt);
    tdKgdKaccum.data() =
        tdKgdKaccum.data() + (-int(kBlockN * params.d_rounded));
#pragma unroll
    for (int i = 0; i < size(acc_dk); ++i) {
      atomicAdd(&tdKgdKaccum(i), acc_dk(i));
    }
...
#pragma unroll
  ...
  // Clear_OOB_K must be false since we don't want to write zeros to gmem
  flash::copy</*Is_even_MN=*/false, Is_even_K, /*Clear_OOB_MN=*/false,
              /*Clear_OOB_K=*/false>(gmem_tiled_copy_dQ, tdQrdQ, tdQgdQ, tdQcdQ,
                                     tdQpdQ,
                                     binfo.actual_seqlen_q - m_block * kBlockM);
}
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          bool Has_alibi, bool Is_even_M, bool Is_even_K, typename Params>
inline __device__ void compute_dq_dk_dv(const Params &params) {
  ...
  if (n_block_max == 1) {
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi,
                               Is_even_M, Is_even_K, true, true>(params, bidb,
                                                                 bidh, 0);
  } else {
    // Iterating backward from n_block_max - 1 to 0 might save 1 register
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi,
                               Is_even_M, Is_even_K, true, false>(
        params, bidb, bidh, n_block_max - 1);
    for (int n_block = n_block_max - 2; n_block > 0; n_block--) {
      compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal,
                                 Has_alibi, Is_even_M, Is_even_K, false, false>(
          params, bidb, bidh, n_block);
    }
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi,
                               Is_even_M, Is_even_K, false, true>(params, bidb,
                                                                  bidh, 0);
  }
}
...
inline __device__ void compute_dq_dk_dv_seqk_parallel(const Params &params) {
  ...
  for (int n_block = blockIdx.x;
       ...
       (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN;
       n_block += gridDim.x) {
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_local,
                               Has_alibi, Is_even_MN, Is_even_K, false, false,
...
inline __device__ void compute_dq_dk_dv_seqq_parallel(const Params &params) {
  ...
  compute_dq_dk_dv_1rowblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi,
                             Is_even_N, Is_even_K>(params, bidb, bidh, m_block);
}
