...
template <bool Is_even_K = true, typename Engine0, typename Layout0,
          ...
          typename Layout2, typename Engine3, typename Layout3>
inline __device__ void
copy_w_min_idx(Tensor<Engine0, Layout0> const &S, Tensor<Engine1, Layout1> &D,
...
template <bool Is_even_K = true, bool Clear_OOB_K = true, typename Engine0,
          ...
          typename Layout3>
inline __device__ void copy_rotary_interleaved(
...
template <bool Is_even_K = true, bool Clear_OOB_K = true, typename Engine0,
          ...
          typename Engine2, typename Layout2, typename Engine3,
          typename Layout3>
inline __device__ void copy_rotary_contiguous(
    Tensor<Engine0, Layout0> const &S, Tensor<Engine1, Layout1> &D,
    Tensor<Engine2, Layout2> const &Cos, Tensor<Engine2, Layout2> const &Sin,
    Tensor<Engine3, Layout3> const &identity_MN, const int max_MN,
    const int min_MN, const int dim, const int rotary_dim) {
      ...
      for (int k = 0; k < size<2>(S); ++k) {
        if (Is_even_K || get<1>(identity_MN(0, 0, k)) < dim) {
          ...
          if (get<1>(identity_MN(0, 0, k)) < rotary_dim) {
            ...
            Tensor gCos = make_tensor(Cos(_, m, k).data() +
                                          (is_left ? 0 : -rotary_dim / 2),
                                      Cos(_, m, k).layout());
            Tensor gSin = make_tensor(Sin(_, m, k).data() +
                                          (is_left ? 0 : -rotary_dim / 2),
                                      Sin(_, m, k).layout());
