...
from torch.fx.experimental.symbolic_shapes import (hint_int, free_symbols,
...
from . import config
from .fx_utils import fx_graph_cse, get_aten_target
from .byteir_fusible_pattern import get_byteir_recompute_nodes
...
def has_recomputable_rng_ops(fx_g):
    for node in fx_g.graph.nodes:
        if must_recompute(node) and hasattr(
                node.target, "tags"
...
def _extract_graph_with_inputs_outputs(joint_graph, inputs, outputs):
    ...
    for node in joint_graph.nodes:
        if node in inputs:
            ...
            all_args = [
                isinstance(env[x], InvalidNodeBase) for x in all_args
    ...
    for x in outputs:
        if isinstance(x, fx.Node):
            ...
            assert not isinstance(
                env[x],
                InvalidNodeBase), f"Node {x} was invalid, but is output"
...
def _is_bwd_seed_offset(node):
    return node.op == "placeholder" and ("bwd_seed" in node.target
                                         or "bwd_base_offset" in node.target)


def _is_fwd_seed_offset(node):
    return node.op == "placeholder" and ("fwd_seed" in node.target
                                         or "fwd_base_offset" in node.target)
...
def _extract_fwd_bwd_modules(joint_module: fx.GraphModule, saved_values,
                             saved_sym_nodes, *, num_fwd_outputs):
    ...
    bwd_seed_offset_inputs = list(
        filter(_is_bwd_seed_offset, joint_module.graph.nodes))
    ...
    bwd_graph = _extract_graph_with_inputs_outputs(
        joint_module.graph, saved_sym_nodes + saved_values + tangent_inputs +
        bwd_seed_offset_inputs, bwd_outputs)
...
def default_partition(
    ...
    return _extract_fwd_bwd_modules(joint_module,
                                    saved_values,
...
def _size_of(node: fx.Node) -> int:
    if 'val' in node.meta:
        ...
        if isinstance(val, py_sym_types):
            ...
            return sum(
                _tensor_nbytes(hint_int(n.numel()), n.dtype) for n in val
...
def get_depth(node, depth_map):
    ...
    arg_depths = [
        get_depth(arg, depth_map) for arg in node.all_input_nodes
        if isinstance(arg, torch.fx.node.Node)
    ]
...
def sort_depths(args, depth_map):
    arg_depths = {
        arg: depth_map[arg]
        for arg in args if isinstance(arg, torch.fx.node.Node)
    }
...
def functionalize_rng_ops(joint_module, fw_module, bw_module, num_sym_nodes):
    ...
    def get_rng_ops(gmod):
        ...
        for node in gmod.graph.nodes:
            if (node.op == "call_function" and hasattr(node.target, "tags")
                    and torch.Tag.nondeterministic_seeded in node.target.tags):
    ...
    for base_node, node_pair in recomputable_rng_ops_map.items():
        ...
        with fw_graph.inserting_before(fw_node):
            functional_fw_node = fw_graph.create_node("call_function",
                                                      run_and_save_rng,
                                                      args=(fw_node.target,
                                                            *fw_node.args),
                                                      kwargs=fw_node.kwargs)
            state = fw_graph.create_node("call_function",
                                         operator.getitem,
                                         args=(functional_fw_node, 0),
                                         kwargs={})
            rng_output = fw_graph.create_node("call_function",
                                              operator.getitem,
                                              args=(
                                                  functional_fw_node,
                                                  1,
        ...
        with bw_graph.inserting_before(bw_node):
            rng_output = bw_graph.create_node("call_function",
                                              run_with_rng_state,
                                              args=(bw_rng_state_node,
                                                    bw_node.target,
                                                    *bw_node.args),
    ...
    fw_output_node = [
        node for node in fw_module.graph.nodes if node.op == "output"
    ][0]
    fw_outputs = fw_output_node.args[0]
    sym_node_start_idx = len(fw_outputs) - num_sym_nodes
    outputs = fw_outputs[:
                         sym_node_start_idx] + fw_rng_state_outputs + fw_outputs[
                             sym_node_start_idx:]
    ...
    for node in joint_module.graph.nodes:
        if must_recompute(node):
            for user in node.users:
                if must_recompute(user) and user.meta["recompute"] > node.meta[
                        "recompute"]:
                    node.meta["recompute"] = 0
    return joint_module


def fuse_aware_min_cut_partition(
    ...
    orig_fw_outputs, required_fw_nodes, required_bw_nodes, unclaimed_nodes = classify_nodes(
        joint_module)
    ...
    if len(required_bw_nodes) == 0:
        return default_partition(joint_module,
                                 _joint_inputs,
                                 num_fwd_outputs=num_fwd_outputs)
    ...
    default_recomputable_ops = [
        ...
        aten.addcmul, aten.gelu, aten.gelu_backward, aten.sum, aten.mean,
        aten._grad_sum_to_size, aten.sum_to_size, aten.amax, aten.to,
        aten.type_as, operator.getitem, aten.squeeze, aten.unsqueeze,
        aten.rsub, aten._to_copy
    ]  # noqa: E501,B950
    view_ops = [aten.squeeze, aten.unsqueeze, aten.alias]
    if compiler == "inductor":
        default_recomputable_ops += [
            prims.div, prims.convert_element_type, aten.clone, aten._to_copy,
            aten.full_like, prims.var, prims.sum, aten.var, aten.std,
            prims.broadcast_in_dim, aten.select, aten.permute,
            aten._unsafe_view, aten.view, aten.expand, aten.slice,
            aten.reshape, aten.broadcast_tensors, aten.scalar_tensor,
            aten.ones, aten.new_zeros, aten.lift_fresh_copy, aten.arange,
            aten.triu, aten.var_mean, aten.isinf, aten.any, aten.full,
            aten.as_strided, aten.zeros, aten.argmax, aten.maximum, aten.cumsum
        ]  # noqa: E501,B950
        view_ops += [
            aten.view, aten.slice, aten.permute, aten.t, aten.transpose,
            prims.broadcast_in_dim, aten.expand, aten.as_strided
        ]
    ...
    recomputable_ops = set(
        recomputable_ops) if recomputable_ops is not None else set(
            default_recomputable_ops)

    random_ops = [aten.native_dropout, aten.rand_like, aten.randn_like]
    compute_intensive_ops = [
        aten.mm, aten.convolution, aten.convolution_backward, aten.bmm,
        aten.addmm, aten.upsample_bilinear2d, aten._softmax,
        aten._softmax_backward_data, aten.native_layer_norm,
        aten.native_layer_norm_backward, aten.native_batch_norm,
        aten.native_batch_norm_backward, aten._native_batch_norm_legit
    ]  # noqa: E501,B950
    ...
    # The node match fusible pattern in byteir, it shoudle be recomputed.
    byteir_recompute_nodes = get_byteir_recompute_nodes(
        full_bw_graph, required_fw_nodes)
        ...
        while len(cur_nodes) > 0:
            ...
            for user in cur.users:
                if user not in required_fw_nodes and not is_fusible(cur, user):
                    return True
                if user not in required_fw_nodes and get_aten_target(
                        user) in view_ops:
                    cur_nodes.add(user)
    ...
    def ban_recomputation(node):
        if node in byteir_recompute_nodes:
            return False
        if "recompute" in node.meta:
            ...
            if node.target == operator.getitem:
                return False
            if node.target in [
                    aten.lift_fresh_copy.default, aten.lift_fresh.default
            ]:
                return False
    ...
    def is_byteir_fusible(node):
        if get_aten_target(node) in [aten.transpose, aten.t]:
            return all(
                get_aten_target(user) in compute_intensive_ops
                for user in node.users)
        return False
    ...
    for node in full_bw_graph.nodes:
        ...
        is_non_tensor_node = (
            ...
            or ('val' in node.meta
                and not isinstance(node.meta['val'], torch.Tensor)))
    ...
    for node in byteir_recompute_nodes:
        nx_graph.add_edge(node.name + "_in", "sink", capacity=math.inf)
    ...
    if AOT_PARTITIONER_DEBUG:
        print("Theoretical Activations Stored: ",
              sum([_size_of(i) for i in saved_values]) / 1e9)
        fw_module_nodes = {
            node.name
            for node in fw_module.graph.nodes if node.op == 'call_function'
        }
        bw_module_nodes = {
            node.name
            for node in bw_module.graph.nodes if node.op == 'call_function'
        }
...
def draw_graph(traced: torch.fx.GraphModule,
               fname: str,
