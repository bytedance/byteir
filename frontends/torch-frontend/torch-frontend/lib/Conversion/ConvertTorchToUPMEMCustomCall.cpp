//===- ConvertTorchToUPMEMCustomCall.cpp ---------------------------*--- C++
//-*-===//
//
...
public:
  ...
  matchAndRewrite(AtenAddOp op, OpAdaptor adaptor,
    ...
    attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                       rewriter.getStringAttr(getAddUpmemName()));
    ...
    customCallOp->setAttr("device", rewriter.getStringAttr("upmem"));
...
public:
  ...
  matchAndRewrite(AtenAddIntOp op, OpAdaptor adaptor,
    ...
    auto attrs = getDefaultAttrs(rewriter);
    attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                       rewriter.getStringAttr(getAddScalarUpmemName()));
    attrs.emplace_back(rewriter.getStringAttr(getCustomCallAttrName()),
                       rewriter.getDictionaryAttr(byteir_attrs));
  ...
  matchAndRewrite(AtenAddScalarOp op, OpAdaptor adaptor,
    ...
    if ((a.getType().cast<ShapedType>().getElementType().isa<Float16Type>())) {
      attrs.emplace_back(
          ...
          rewriter.getStringAttr(getAddScalarUpmemName() + ".fp16"));
    } else if ((a.getType()
                    .cast<ShapedType>()
                    .getElementType()
                    .isa<Float32Type>())) {
      ...
      attrs.emplace_back(
          rewriter.getStringAttr("call_target_name"),
          rewriter.getStringAttr(getAddScalarUpmemName() + ".fp32"));
    } else {
      attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                         rewriter.getStringAttr(getAddScalarUpmemName()));
    }
...
public:
  ...
  matchAndRewrite(AtenAddTensorOp op, OpAdaptor adaptor,
    ...
    byteir_attrs.emplace_back(rewriter.getStringAttr("device"),
                              rewriter.getStringAttr("upmem"));
    auto attrs = getDefaultAttrs(rewriter);

    // check if a is float
    if ((a.getType().cast<ShapedType>().getElementType().isa<Float16Type>())) {
      attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                         rewriter.getStringAttr(getAddUpmemName() + ".fp16"));
    } else if ((a.getType()
                    .cast<ShapedType>()
                    .getElementType()
                    .isa<Float32Type>())) {
      ...
      attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                         rewriter.getStringAttr(getAddUpmemName() + ".fp32"));
    } else {
      attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                         rewriter.getStringAttr(getAddUpmemName()));
    }
    ...
    //              .isa<FloatType>()) {
    //    attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
    //                      rewriter.getStringAttr(getAddScalarUpmemName()+".float"));
    //   } else {
    //  attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
    //                      rewriter.getStringAttr(getAddScalarUpmemName()));
    //   }
...
public:
  ...
  matchAndRewrite(AtenMulTensorOp op, OpAdaptor adaptor,
    ...
    auto attrs = getDefaultAttrs(rewriter);
    attrs.emplace_back(rewriter.getStringAttr("call_target_name"),
                       rewriter.getStringAttr(getMulUpmemName()));
    attrs.emplace_back(rewriter.getStringAttr(getCustomCallAttrName()),
                       rewriter.getDictionaryAttr(byteir_attrs));
    auto customCallOp = rewriter.create<mhlo::CustomCallOp>(
        op->getLoc(), resultTypes, bufferArgs, ArrayRef<NamedAttribute>{attrs});
...
public:
  ...
  void runOnOperation() override {
    ...
    patterns.add<ConvertAtenMulTensorOp>(typeConverter, context);
...
mlir::createConvertTorchToUPMEMCustomCall() {
  return std::make_unique<ConvertTorchToUPMEMCustomCall>();
}
