...
// CHECK:         %[[V5:.*]] = "stablehlo.scatter"(%[[V0]], %[[V3]], %[[V4]]) ({
// CHECK:         ^bb0(%[[ARG3:.*]]: tensor<f32>, %[[ARG4:.*]]: tensor<f32>):
// CHECK:           %[[V7:.*]] = stablehlo.add %[[ARG3]], %[[ARG4]] : tensor<f32>
// CHECK:           stablehlo.return %[[V7]] : tensor<f32>
// CHECK:         }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [1], inserted_window_dims = [0], scatter_dims_to_operand_dims = [0], index_vector_dim = 1>, unique_indices = false} : (tensor<10x8xf32>, tensor<10x1xi64>, tensor<10x8xf32>) -> tensor<10x8xf32>
