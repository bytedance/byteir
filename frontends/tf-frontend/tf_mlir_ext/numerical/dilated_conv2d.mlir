// RUN: tf-ext-opt -tfl-identify-dilated-conv %s -o %t0
// RUN: FileCheck %s < %t0
// RUN: python3 numerical_test.py %s %t0
// RUN: tf-ext-opt -tfl-identify-dilated-conv -xla-legalize-tf -canonicalize %s -o %t1
// RUN: FileCheck %s < %t1 --check-prefix MHLO
// RUN: python3 numerical_test.py %s %t1 --config dilated_conv2d
