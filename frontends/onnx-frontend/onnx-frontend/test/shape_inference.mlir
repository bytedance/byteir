...
//===----------------------------------------------------------------------===//
/// Test the default behavior of shape inference when onnx and stablehlo
/// dialects are mixed together.
//===----------------------------------------------------------------------===//
...
module {
  func.func @test_onnx_with_custom_call(%arg0: tensor<10x10xf32>) -> tensor<10xf32> {
    ...
    %2 = "stablehlo.custom_call"(%1) {byteir_attrs = {axis = 3 : i64, keep_dims = false, select_last_index = false}, call_target_name = "byteir.arg_max", called_computations = [], has_side_effect = false} : (tensor<10x10xf32>) -> tensor<10xf32>
...
// CHECK-NEXT:     %2 = stablehlo.custom_call @byteir.arg_max(%1) {byteir_attrs = {axis = 3 : i64, keep_dims = false, select_last_index = false}} : (tensor<10x10xf32>) -> tensor<10xf32>
