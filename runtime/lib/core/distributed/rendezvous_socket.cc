...
char *GetHostIP() {
  const char *device = getenv("BRT_NET_DEVICE");
  if (device and strcmp(device, "lo") == 0) {
    BRT_LOGS_DEFAULT(ERROR) << "illegal net device: lo\n";
    BRT_THROW("invalid argument");
  }

  struct ifaddrs *ifa;
  assert(getifaddrs(&ifa) != -1);
  ...
  for (struct ifaddrs *p = ifa; p != NULL; p = p->ifa_next) {
    if (p->ifa_addr and p->ifa_addr->sa_family == AF_INET and p->ifa_name) {
      ...
      if (strcmp(name, "lo") != 0 and
        ...
        BRT_LOGS_DEFAULT(INFO)
            << "using net device " << name << ", " << host_ip;
  ...
  if (device) {
    BRT_LOGS_DEFAULT(ERROR) << "failed to get host ip for device " << device;
  } else {
    BRT_LOGS_DEFAULT(ERROR) << "failed to get host ip";
  }
  BRT_THROW("system error");
  return nullptr;
}
