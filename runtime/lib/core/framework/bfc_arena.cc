...
BFCArena::BFCArena(std::unique_ptr<IAllocator> resource_allocator,
                   size_t total_memory, bool initial_max_extend,
  ...
  // static_cast<std::underlying_type_t<ArenaExtendStrategy>>(arena_extend_strategy);
  // doesn't work on this compiler

  curr_region_allocation_bytes_ = RoundedBytes(
      std::min(total_memory, static_cast<size_t>(initial_chunk_size_bytes_)));
  ...
  // We never want to shrink the initial allocation if the arena extend strategy
  // is kNextPowerOfTwo. This could seem confusingly arbitrary but the rationale
  // is as follows: The user selected initial allocation chunk is only valid for
  // the arena extend strategy kNextPowerOfTwo and the user has likely chosen
  // this initial value so that any ad-hoc arena extensions/shrinkages could
  // potentially be avoided. So we do not consider the initial allocation for
  // shrinkage whatever its usage status. On the other hand, if the arena
  // extension strategy is kSameAsRequested, any initial chunk set by the user
  // or otherwise, is moot and the arena will only extend based on the request
  // size. In these cases, we consider any allocation for shrinkage if it is
  // left unused (even if it is the first allocation).
  if (arena_extend_strategy_ == ArenaExtendStrategy::kSameAsRequested) {
    // Consider all allocation regions (including first allocation region) for
    // shrinkage
  ...
  for (BinNum b = 0; b < kNumBins; b++) {
    ...
    BRT_ENFORCE(BinForSize(bin_size) == BinFromIndex(b));
    BRT_ENFORCE(BinForSize(bin_size + 255) == BinFromIndex(b));
    BRT_ENFORCE(BinForSize(bin_size * 2 - 1) == BinFromIndex(b));
    if (b + 1 < kNumBins) {
      BRT_ENFORCE(BinForSize(bin_size * 2) != BinFromIndex(b));
    }
  }
...
BFCArena::Chunk *BFCArena::ChunkFromHandle(ChunkHandle h) {
  BRT_ENFORCE(h < chunks_.size());
  return &(chunks_[h]);
}

Status BFCArena::Extend(size_t rounded_bytes) {
  size_t available_bytes =
      memory_limit_ - static_cast<size_t>(stats_.total_allocated_bytes);
  ...
  if (rounded_bytes > available_bytes) {
    return BRT_MAKE_STATUS(BRT, FAIL, "Available memory of ", available_bytes,
  ...
  auto safe_alloc = [this](size_t alloc_bytes) {
    ...
    BRT_TRY { new_mem = device_allocator_->Alloc(alloc_bytes); }
    BRT_CATCH(const std::bad_alloc &) {
    ...
    BRT_CATCH(const BrtException &brt_exception) {
      ...
      BRT_HANDLE_EXCEPTION([&brt_exception]() {
        if (std::string(brt_exception.what()).find("cudaMalloc") ==
                std::string::npos &&
            std::string(brt_exception.what()).find("hipMalloc") ==
                std::string::npos) {
          BRT_RETHROW;
        }
      });
  ...
  auto get_extend_bytes = [this,
                           available_bytes](const size_t bytes) -> size_t {
    ...
    } else if (arena_extend_strategy_ ==
               ArenaExtendStrategy::kSameAsRequested) {
      // BFC Arena could cause internal and external fragmentation. But, running
      // training with big batch size will be very sensitive to fragmentation.
      // So, to avoid fragmentation, just extend arena with actual requested
      // size.
      extend_bytes = bytes;
    } else {
      BRT_THROW("Incorrect arena extend strategy.",
                static_cast<int32_t>(arena_extend_strategy_));
    }
  ...
  while (mem_addr == nullptr) {
    ...
    // give up if we can't satisfy the requested size, or we're attempting an
    // allocation of less than 8K.
    //
    // the latter protects against an infinite loop that occurs when bytes is
    // less than 2560. at that point the 10% reduction to 2304 bytes is undone
    // by rounding to a 256 boundary in RoundedBytes, leading to an infinite
    // loop. the 8K value is just to give up a little earlier vs. getting all
    // the way down to 2560 bytes. If we can't allocate 8K, we're pretty much
    // dead.
  ...
  if (mem_addr == nullptr) {
    return BRT_MAKE_STATUS(
        BRT, FAIL, "Failed to allocate memory for requested buffer of size ",
        rounded_bytes);
  }
...
size_t BFCArena::RoundedBytes(size_t bytes) {
  ...
  BRT_ENFORCE(size_t{0} == rounded_bytes % kMinAllocationSize);
  return rounded_bytes;
}

void *BFCArena::Alloc(size_t size) { return AllocateRawInternal(size, false); }
...
void *BFCArena::AllocateRawInternal(size_t num_bytes,
  ...
  if (status.IsOK()) {
    ...
    if (ptr != nullptr) {
      ...
      status = BRT_MAKE_STATUS(BRT, FAIL,
  ...
  for (; bin_num < kNumBins; bin_num++) {
    ...
    for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end();
      ...
      if (chunk->size >= rounded_bytes) {
        ...
        stats_.max_alloc_size = std::max<int64_t>(
            stats_.max_alloc_size, static_cast<int64_t>(chunk->size));
        return chunk->ptr;
      }
    }
  }
...
void BFCArena::SplitChunk(BFCArena::ChunkHandle h, size_t num_bytes) {
  ...
  Chunk *c = ChunkFromHandle(h);
  BRT_ENFORCE(!c->in_use() && (c->bin_num == kInvalidBinNum));
...
void BFCArena::Merge(BFCArena::ChunkHandle h1, BFCArena::ChunkHandle h2) {
  Chunk *c1 = ChunkFromHandle(h1);
  Chunk *c2 = ChunkFromHandle(h2);
  // We can only merge chunks that are not in use.
  BRT_ENFORCE(!c1->in_use() && !c2->in_use());
...
void BFCArena::RemoveFreeChunkFromBin(BFCArena::ChunkHandle h) {
  Chunk *c = ChunkFromHandle(h);
  BRT_ENFORCE(!c->in_use() && (c->bin_num != kInvalidBinNum));
  BRT_ENFORCE(BinFromIndex(c->bin_num)->free_chunks.erase(h) > 0,
  ...
  for (const auto &region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      ...
      if (c->in_use()) {
        ...
        BRT_ENFORCE(bin->free_chunks.count(h) == 1);
        BRT_ENFORCE(c->bin_num == bin_num);
      }
      h = c->next;
    }
  }
