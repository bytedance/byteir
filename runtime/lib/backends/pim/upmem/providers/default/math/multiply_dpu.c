...
// subtract
static void subtract(T *bufferC, T *bufferA, T *bufferB) {
...
int main() {
  ...
  // subtractress of the current row in MRAM
  uint32_t mram_base_subtractr_A =
      (uint32_t)(DPU_MRAM_HEAP_POINTER + start_row * n_size * sizeof(T));
  uint32_t mram_base_subtractr_B =
      (uint32_t)(DPU_MRAM_HEAP_POINTER + max_rows * n_size_pad * sizeof(T));
  uint32_t mram_base_subtractr_C =
      (uint32_t)(DPU_MRAM_HEAP_POINTER + max_rows * n_size_pad * sizeof(T) +
                 n_size_pad * sizeof(T) + start_row * sizeof(T));
  uint32_t mram_temp_subtractr_A = mram_base_subtractr_A;
  uint32_t mram_temp_subtractr_B = mram_base_subtractr_B;
  ...
  for (unsigned int i = start_row; i < start_row + rows_per_tasklet;
    ...
    mram_temp_subtractr_A =
        (uint32_t)(DPU_MRAM_HEAP_POINTER + i * n_size * sizeof(T));
    mram_temp_subtractr_B = mram_base_subtractr_B;
    ...
    for (n = 0; n < (int32_t)(n_size - (BLOCK_SIZE / sizeof(T)));
      ...
      mram_read((__mram_ptr void const *)(mram_temp_subtractr_A), cache_A,
                BLOCK_SIZE);
      mram_read((__mram_ptr void const *)(mram_temp_subtractr_B), cache_B,
                BLOCK_SIZE);
      ...
      if (offset) {
        ...
        mram_read((__mram_ptr void const *)(mram_temp_subtractr_A + BLOCK_SIZE),
                  cache_A_aux, 8);
      ...
      // Compute subtract
      subtract(cache_C, cache_A, cache_B);

      // Update memory subtractresses
      mram_temp_subtractr_A += BLOCK_SIZE;
      mram_temp_subtractr_B += BLOCK_SIZE;
    }
    ...
    mram_write(cache_C, (__mram_ptr void *)(mram_base_subtractr_C), 8);

    // Update memory subtractress
    // mram_base_subtractr_C += 2 * sizeof(T);
    mram_base_subtractr_C += 8;
  }
