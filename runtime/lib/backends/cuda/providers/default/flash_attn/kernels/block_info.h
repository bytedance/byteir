...
template <bool Varlen = true> struct BlockInfo {
  ...
  __device__ BlockInfo(const Params &params, const int bidb)
      : sum_s_q(!Varlen || params.cu_seqlens_q == nullptr
        ...
        actual_seqlen_k(!Varlen || params.cu_seqlens_k == nullptr
                            ? params.seqlen_k
                            : params.cu_seqlens_k[bidb + 1] - sum_s_k) {}

  template <typename index_t>
  inline __device__ index_t q_offset(const index_t batch_stride,
  ...
  template <typename index_t>
  inline __device__ index_t k_offset(const index_t batch_stride,
                                     const index_t row_stride,
                                     const int bidb) const {
