...
template <typename T> inline __device__ uint32_t relu2(const uint32_t x);
...
template <typename T> inline __device__ uint32_t convert_relu2(const float2 x);
...
template <typename T> inline __device__ float2 half2_unpack(uint32_t a);

template <> inline __device__ float2 half2_unpack<__half>(uint32_t a) {
  return __half22float2(reinterpret_cast<__half2(&)>(a));
}

#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
template <> inline __device__ float2 half2_unpack<__nv_bfloat16>(uint32_t a) {
...
// Converted two vectors of 8 half's or bf16's into float, then take their dot
// product.
template <typename T>
...
gemm(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsA,
...
template <typename Tensor0, typename Tensor1, typename Tensor2,
          ...
          typename ThrCopy>
inline __device__ void
gemm_A_in_regs(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsB,
...
inline __device__ auto convert_layout_acc_rowcol(Layout acc_layout) {
  ...
  // TD [2023-08-13]: Idk why but get<0, 1>(l) doesn't work for Cutlass 3.2, I'm
  // getting "int_tuple.hpp(74): error: conversion to inaccessible base class"
  // return make_layout(make_layout(get<0, 1>(l), get<1>(l)), make_layout(get<0,
  // 0>(l), get<2>(l)));
...
// Convert rowcol_layout from (nrow=(2, MMA_M), ncol=(2, MMA_N)) to ((2, 2, 2),
// MMA_M, MMA_N / 2) if using m16n8k16, or to ((2, 2, 1), MMA_M, MMA_N) if using
// m16n8k8.
...
inline __device__ auto convert_layout_rowcol_Aregs(Layout rowcol_layout) {
  ...
  auto l = logical_divide(
      rowcol_layout,
      Shape<X, Shape<X, Int<MMA_N_divisor>>>{}); // ((2, MMA_M), (2, (2, MMA_N /
                                                 // 2)))
  // TD [2023-08-13]: Same error as above on Cutlass 3.2
  // return make_layout(make_layout(get<1, 0>(l), get<0, 0>(l), get<1, 1,
  // 0>(l)),
  //                    get<0, 1>(l),
  //                    get<1, 1, 1>(l));
