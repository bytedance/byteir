...
dot_do_o(Tensor<Engine0, Layout0> const &do_, Tensor<Engine0, Layout0> const &o,
...
#pragma unroll
    ...
    if (threadIdx.x % THREADS_PER_ROW == 0) {
      ...
      // recast<float>(sdPsum)(mi * gdP_col_stride + threadIdx.x /
      // THREADS_PER_ROW) = dP_sum;
    }
  }
}
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          bool Is_even_MN, bool Is_even_K, bool Is_first, bool Is_last,
...
compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh,
  ...
  if (n_block * kBlockN >= binfo.actual_seqlen_k || binfo.actual_seqlen_q == 0)
    return;
  ...
  Tensor sdPsum =
      make_tensor(make_smem_ptr(reinterpret_cast<float2 *>(
                      (sP.data() + cute::max(size(sP), size(sdQ))).get())),
...
#pragma unroll
  ...
  // Otherwise we get wrong result for the case where we don't enter the for
  // loop. And we might read OOB elements from gQ and gdO.
...
#pragma unroll
    ...
    if (Is_dropout) {
      ...
      uint32_t block_row_idx =
          m_block * (kBlockM / 16) + warp_id % AtomLayoutMS;
...
#pragma unroll
    ...
    tdQgdQaccum.data() =
        tdQgdQaccum.data() + (-int(kBlockM * params.d_rounded));
    if (Is_first || Seq_parallel) {
      clear(acc_dq);
    } else {
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          bool Is_even_N, bool Is_even_K, typename Params>
inline __device__ void
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          bool Is_even_M, bool Is_even_K, typename Params>
inline __device__ void compute_dq_dk_dv(const Params &params) {
  ...
  if (n_block_max == 1) {
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_even_M,
                               Is_even_K, true, true>(params, bidb, bidh, 0);
  } else {
    // Iterating backward from n_block_max - 1 to 0 might save 1 register
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_even_M,
                               Is_even_K, true, false>(params, bidb, bidh,
                                                       n_block_max - 1);
    for (int n_block = n_block_max - 2; n_block > 0; n_block--) {
      compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal,
                                 Is_even_M, Is_even_K, false, false>(
          params, bidb, bidh, n_block);
    }
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_even_M,
                               Is_even_K, false, true>(params, bidb, bidh, 0);
  }
}
...
template <typename Kernel_traits, bool Is_dropout, bool Is_causal,
          bool Is_even_MN, bool Is_even_K, typename Params>
inline __device__ void compute_dq_dk_dv_seqk_parallel(const Params &params) {
  ...
  compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_even_MN,
                             Is_even_K, false, false, /*Seq_parallel=*/true>(
      params, bidb, bidh, n_block);
}
...
inline __device__ void compute_dq_dk_dv_seqq_parallel(const Params &params) {
  ...
  compute_dq_dk_dv_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_even_N,
                             Is_even_K>(params, bidb, bidh, m_block);
}
