...
namespace brt {
/**
   CodeLocation captures information on where in the source code a message came
   from.
*/
struct CodeLocation {
  /**
  ...
  */
  ...
  std::string FileNoPath() const {
    // assuming we always have work to do, so not trying to avoid creating a new
    // string if no path was removed.
