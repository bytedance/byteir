...
public:
  ...
  BFCArena(
      std::unique_ptr<IAllocator> resource_allocator, size_t total_memory,
      bool initial_max_extend = false,
  ...
  size_t Max() const override { return memory_limit_; }
...
private:
  ...
  struct Bin {
    ...
    struct ChunkComparator {
      explicit ChunkComparator(BFCArena *allocator) : allocator_(allocator) {}
  ...
  private:
    ...
    // (May be used by the client to track which allocation region was allocated
    ...
    BRT_DISALLOW_ASSIGNMENT(AllocationRegion);
  };
  ...
  public:
    ...
    void RemoveAllocationRegion(void *ptr) {
      ...
      BRT_ENFORCE(entry != regions_.end(), "Could not find Region for: ", ptr);
  ...
  private:
    BRT_DISALLOW_COPY_ASSIGNMENT_AND_MOVE(RegionManager);
  ...
  BRT_DISALLOW_COPY_ASSIGNMENT_AND_MOVE(BFCArena);
};
#ifdef __GNUC__
