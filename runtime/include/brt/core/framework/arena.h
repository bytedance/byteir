...
// The setting like max_chunk_size is init by IDeviceDescriptor from resource
// allocator
...
struct AllocatorStats {
  ...
  int64_t num_reserves; // Number of reserves. (Number of calls to Reserve() in
                        // arena-based allocators)
  int64_t num_arena_extensions; // Number of arena extensions (Relevant only for
                                // arena based allocators)
  int64_t num_arena_shrinkages; // Number of arena shrinkages (Relevant only for
                                // arena based allocators)
  int64_t bytes_in_use;         // Number of bytes in use.
  int64_t total_allocated_bytes; // The total number of allocated bytes by the
                                 // allocator.
  int64_t max_bytes_in_use;      // The maximum bytes in use.
  int64_t max_alloc_size;        // The max single allocation seen.
                          // The upper limit what the allocator can allocate, if
                          // such a limit is known. Certain allocator may return
                          // 0 to indicate the limit is unknown.
  int64_t bytes_limit;
