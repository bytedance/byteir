...
{
  public:
    HBMPIMKernel(shared_ptr<MultiChannelMemorySystem> mem, int num_pim_chan, int num_pim_rank)
        : mem_(mem),
    ...
    void preprocessSrf(TensorBurstType* input_npbst, fp16** params, int burst_offset,
                       int num_srf_usage);
    */
    /*
    void programSrf();
    */
    ...
    void preloadGemv(TensorBurstType* operand, unsigned starting_row = 0, unsigned starting_col = 0);
    void preloadNoReplacement(TensorBurstType* operand, unsigned startingRow, unsigned startingCol);
    /*
    void preloadEltwise(TensorBurstType* operand, pimBankType bank_types, unsigned startingRow,
                        unsigned startingCol);
    */
    void executeGemv(TensorBurstType* w_data, TensorBurstType* i_data, bool is_tree);
    void executeEltwise(int dim, pimBankType bank_types, KernelType ktype, int input0_row,
                        int result_row, int input1_row = 0);
    void computeGemv(TensorBurstType* data, int num_input_tiles, int num_output_tile, int input_tile,
