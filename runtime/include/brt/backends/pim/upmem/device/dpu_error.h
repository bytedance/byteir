...
/**
 * @brief Status returned by any API operation to specify the success or failure
 * of the operation.
 */
typedef enum dpu_error_t {
  ...
  DPU_ERR_MRAM_BUSY,
  /** Error returned when trying to prepare a memory transfer on an already
     prepared dpu. */
  ...
  /** Error returned when trying to do a multiple-rank operation with 0 thread
     per rank */
  DPU_ERR_NO_THREAD_PER_RANK,
  /** Error returned when trying to serialize a ctx in a buffer with an
     incorrect size */
  ...
  /** Error returned when the DPU program uses more tasklets than the hardware
     has threads */
  DPU_ERR_TOO_MANY_TASKLETS,
  /** Error returned when the number of scatter gather transfer blocks exceeds
     the maximal number of blocks */
  DPU_ERR_SG_TOO_MANY_BLOCKS,
  /** Error returned when the scatter transfer total number of bytes in prepared
     blocks mismatch with the size of the transfer
     (dpu_push_sg_xfer(..,.length,...)) */
  DPU_ERR_SG_LENGTH_MISMATCH,
  /** Error returned when a scatter gather transfer is performed without prior
     SG transfer activation */
  ...
  /** Error returned if one sg xfer is launched and the sg buffer pool has not
     been initialized */
 ...
 * @return The string associated to the specified status. It is the user
 * responsability to free the returned string when it is not needed anymore.
 */
const char *dpu_error_to_string(dpu_error_t status);

/**
 * @brief In an error from an async job, offset of the error value where the job
