// RUN: byteir-opt %s -byre-host="device-file-name=your_file target=cpu" | FileCheck %s

// CHECK-LABEL: func.func @main
module attributes {byre.container_module} {
  module attributes {byteir.llvm_module} {
    memref.global "private" constant @__constant_1x128xi32 : memref<1x128xi32> = dense<"">
    func.func @Unknown1(%arg0: memref<1xi64>, %arg1: memref<1xi64>, %arg2: memref<1xi64>, %arg3: memref<1x128xi32>) -> (memref<1x128xi32>, memref<1x128xi32>) attributes {__byre__kernel_name = "Unknown1", __byre__llvm_file_name = "host_kernels.ll", __byteir_hlo_aggressive_fusion__, arg_offsets = [0 : i32, 1 : i32, 2 : i32, 3 : i32, 4 : i32, 5 : i32], byre_compute_name = "LLVMJITOp", byre_force_compute_name, llvm.emit_c_interface} {
      %c0 = arith.constant 0 : index
      %c128 = arith.constant 128 : index
      %c1 = arith.constant 1 : index
      %0 = memref.get_global @__constant_1x128xi32 : memref<1x128xi32>
      %alloc = memref.alloc() : memref<1x128xi32>
      %alloc_0 = memref.alloc() : memref<1x128xi32>
      scf.for %arg4 = %c0 to %c128 step %c1 {
        %1 = arith.cmpi slt, %arg4, %c0 : index
        %2 = arith.addi %arg4, %c128 : index
        %3 = arith.select %1, %2, %arg4 : index
        %4 = memref.load %0[%c0, %3] : memref<1x128xi32>
        %5 = memref.load %arg2[%c0] : memref<1xi64>
        %6 = memref.load %arg0[%c0] : memref<1xi64>
        %7 = memref.load %arg1[%c0] : memref<1xi64>
        %8 = arith.addi %6, %7 : i64
        %9 = arith.addi %5, %8 : i64
        %10 = arith.trunci %9 : i64 to i32
        %11 = arith.cmpi slt, %4, %10 : i32
        %12 = arith.extui %11 : i1 to i32
        memref.store %12, %alloc_0[%c0, %3] : memref<1x128xi32>
      }
      scf.for %arg4 = %c0 to %c128 step %c1 {
        %1 = arith.cmpi slt, %arg4, %c0 : index
        %2 = arith.addi %arg4, %c128 : index
        %3 = arith.select %1, %2, %arg4 : index
        %4 = memref.load %alloc_0[%c0, %3] : memref<1x128xi32>
        %5 = memref.load %arg3[%c0, %3] : memref<1x128xi32>
        %6 = arith.muli %4, %5 : i32
        memref.store %6, %alloc[%c0, %3] : memref<1x128xi32>
      }
      return %alloc_0, %alloc : memref<1x128xi32>, memref<1x128xi32>
    }
  }
  func.func @main(%arg0: memref<1xi64> {byre.argname = "Input0", byre.argtype = 1 : i32}, %arg1: memref<1xi64> {byre.argname = "Input1", byre.argtype = 1 : i32}, %arg2: memref<1xi64> {byre.argname = "Input2", byre.argtype = 1 : i32}, %arg3: memref<1x128xi32> {byre.argname = "Input3", byre.argtype = 1 : i32}, %arg4: memref<1x128xi32> {byre.argname = "Output0", byre.argtype = 2 : i32}, %arg5: memref<1x128xi32> {byre.argname = "Output1", byre.argtype = 2 : i32}) attributes {byre.entry_point} {
    byre.compute @LLVMJITOp(%arg0, %arg1, %arg2, %arg3, %arg4, %arg5) {kernel_name = "Unknown1", llvm_file_name = "host_kernels.ll", memory_effects = [1 : i32, 1 : i32, 1 : i32, 1 : i32, 2 : i32, 2 : i32]} : memref<1xi64>, memref<1xi64>, memref<1xi64>, memref<1x128xi32>, memref<1x128xi32>, memref<1x128xi32>
    return
  }
}

