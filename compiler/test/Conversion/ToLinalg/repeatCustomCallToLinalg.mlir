...
func.func @convert_repeat_static(%arg0: tensor<16x96xf16>, %arg1: tensor<16xi64>) -> tensor<128x96xf16> attributes {__byteir_hlo_aggressive_fusion__} {
  %0 = mhlo.custom_call @byteir.repeat(%arg0, %arg1) {ada.op_unique_key = "ada.ref_value.0", backend_config = "", byteir_attrs = {Trepeats = i64}, device = "host"} : (tensor<16x96xf16>, tensor<16xi64>) -> tensor<128x96xf16>
...
// CHECK-LABEL:     %4 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel"]} ins(%extracted_slice : tensor<1x96xf16>) outs(%3 : tensor<?x96xf16>) attrs =  {ada.op_unique_key = "ada.ref_value.0", byteir_attrs = {Trepeats = i64}, device = "host"} {
...
func.func @convert_repeat_output_dynamic(%arg0: tensor<16x96xf16>, %arg1: tensor<16xi64>) -> tensor<?x96xf16> attributes {__byteir_hlo_aggressive_fusion__} {
  %0 = mhlo.custom_call @byteir.repeat(%arg0, %arg1) {ada.op_unique_key = "ada.ref_value.0", backend_config = "", byteir_attrs = {Trepeats = i64}, device = "host"} : (tensor<16x96xf16>, tensor<16xi64>) -> tensor<?x96xf16>
...
// CHECK-LABEL:     %7 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel"]} ins(%extracted_slice : tensor<1x96xf16>) outs(%6 : tensor<?x96xf16>) attrs =  {ada.op_unique_key = "ada.ref_value.0", byteir_attrs = {Trepeats = i64}, device = "host"} {
// CHECK-LABEL:     ^bb0(%in: f16, %out: f16):
// CHECK-LABEL:       linalg.yield %in : f16
// CHECK-LABEL:     } -> tensor<?x96xf16>
...
func.func @convert_repeat_input_dynamic(%arg0: tensor<?x96xf16>, %arg1: tensor<?xi64>) -> tensor<128x96xf16> attributes {__byteir_hlo_aggressive_fusion__} {
  %0 = mhlo.custom_call @byteir.repeat(%arg0, %arg1) {ada.op_unique_key = "ada.ref_value.0", backend_config = "", byteir_attrs = {Trepeats = i64}, device = "host"} : (tensor<?x96xf16>, tensor<?xi64>) -> tensor<128x96xf16>
  return %0 : tensor<128x96xf16>
}
...
func.func @convert_repeat_dynamic(%arg0: tensor<?x96xf16>, %arg1: tensor<?xi64>) -> tensor<?x96xf16> attributes {__byteir_hlo_aggressive_fusion__} {
  %0 = mhlo.custom_call @byteir.repeat(%arg0, %arg1) {ada.op_unique_key = "ada.ref_value.0", backend_config = "", byteir_attrs = {Trepeats = i64}, device = "host"} : (tensor<?x96xf16>, tensor<?xi64>) -> tensor<?x96xf16>
  return %0 : tensor<?x96xf16>
}
