// RUN: byteir-opt %s --transform-insertion="func-anchor=test_func_anchor match-prefix=test_prefix" --split-input-file | FileCheck %s
// RUN: byteir-opt %s --transform-insertion="func-anchor=test_func_anchor match-prefix=test_prefix tile-size-attr-name=test.tile_size tile-interchange-attr-name=test.tile_interchange" --split-input-file | FileCheck %s -check-prefix=TESTATTR
...
func.func @generic_attr(%input: tensor<12x7x25xf32>) -> tensor<12x25xf32> attributes {test_func_anchor, other} {
  ...
  %fill = linalg.fill ins(%five : f32) outs(%init : tensor<12x25xf32>) -> tensor<12x25xf32>
  // CHECK: linalg.generic
  // CHECK-SAME: attrs =  {test.tile_interchange = [0, 1], test.tile_size = [1, 2], test_prefix3} {
  %0 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d2)>],
    iterator_types = ["parallel", "reduction", "parallel"]
...
// CHECK: %0 = transform.structured.match attributes {test_prefix0}
// CHECK: transform.structured.fuse_ext
// CHECK-SAME: {tile_interchange = [], tile_sizes = [1]}

// CHECK: transform.sequence failures(propagate) {
// CHECK: ^bb0(%arg0: !pdl.operation): 
// CHECK: %0 = transform.structured.match attributes {test_prefix1}
// CHECK: transform.structured.fuse_ext
// CHECK-SAME: {tile_interchange = [], tile_sizes = [1]}

// CHECK: transform.sequence failures(propagate) {
// CHECK: ^bb0(%arg0: !pdl.operation): 
// CHECK: transform.structured.match attributes {test_prefix2}
// CHECK: transform.structured.fuse_ext
// CHECK-SAME: {tile_interchange = [], tile_sizes = [1]}

// CHECK: transform.sequence failures(propagate) {
// CHECK: ^bb0(%arg0: !pdl.operation): 
// CHECK: transform.structured.match attributes {test_prefix3}
// CHECK: transform.structured.fuse_ext
// CHECK-SAME: {tile_interchange = [], tile_sizes = [1]}

// TESTATTR: transform.sequence failures(propagate) {
// TESTATTR: ^bb0(%arg0: !pdl.operation): 
// TESTATTR: transform.structured.match attributes {test_prefix3}
