...
struct UserangeInfoBuilder {
  using OperationListT = byteir::Liveness::OperationListT;
  using ValueSetT = BufferViewFlowAnalysis::ValueSetT;

public:
  /// Constructs an Userange builder.
  UserangeInfoBuilder(byteir::Liveness *liveness, ValueSetT values,
                      OperationListT op_list)
      : values(std::move(values)), op_list(std::move(op_list)),
        liveness(liveness) {}

  /// Computes the userange of the current value by iterating over all of its
  /// uses.
  byteir::Liveness::OperationListT computeUserange() {
...
private:
  ...
  Block *findTopLiveBlock(Operation *op) const {
    Operation *topOp = op;
    while (const byteir::LivenessBlockInfo *blockInfo =
  ...
  void addOperationsInBlockAndFindSuccessors(
      ...
      SmallVector<Block *, 8> &blocksToProcess) {
    const byteir::LivenessBlockInfo *blockInfo =
        liveness->getLiveness(toProcess);
  ...
  Operation *getEndOperation(Block *block) {
    const byteir::LivenessBlockInfo *blockInfo = liveness->getLiveness(block);
    if (llvm::any_of(values, [&](Value v) { return blockInfo->isLiveOut(v); }))
      return &block->back();
...
UseInterval::Vector UserangeAnalysis::computeInterval(
    Value value, const byteir::Liveness::OperationListT &operationList) {
...
void UserangeAnalysis::dump(raw_ostream &os) {
  ...
  os << "// ---- Id and Operation Begins -----\n";
  DenseSet<size_t> rangeIds;
  for (auto value : values) {
    for (auto *rangeIt = useIntervalMap[value].begin(),
              *e = useIntervalMap[value].end();
         rangeIt != e; ++rangeIt) {
      rangeIds.insert(rangeIt->start);
      rangeIds.insert(rangeIt->end);
    }
  }
  SmallVector<size_t> sortedRangeIds(rangeIds.begin(), rangeIds.end());
  std::sort(sortedRangeIds.begin(), sortedRangeIds.end(),
            [&](size_t left, size_t right) { return left < right; });
  for (size_t id : rangeIds) {
    os << "ID: " << id << ", Op: ";
    getOperation(id)->print(os);
    os << "\n";
  }
  os << "// ---- Id and Operation Ends -----\n";
