...
#define DEBUG_TYPE "linalg-hoisting-ext"
...
namespace mlir {
namespace linalg {
void hoistRedundantVectorTransfersExt(Operation *root,
                                      bool moveLoopInvariantCodeSelf) {
  bool changed = true;
  while (changed) {
    changed = false;
    ...
    root->walk([&](LoopLikeOpInterface loopLike) {
      if (!moveLoopInvariantCodeSelf && root == loopLike)
        return;
